...
class Applicant(Base):
    ...
    def to_dict(self) -> Dict:
        ...
        try:
            if self.work_experience:
                work_experience = json.loads(self.work_experience) if isinstance(self.work_experience, str) else self.work_experience
        except (json.JSONDecodeError, TypeError):
            work_experience = []
            
        try:
            if self.education:
                education = json.loads(self.education) if isinstance(self.education, str) else self.education
        except (json.JSONDecodeError, TypeError):
            education = []
    ...
    def from_dict(cls, data: Dict) -> 'Applicant':
        ...
        skills_json = json.dumps(data.get('skills', [])) if data.get('skills') else None
        work_exp_json = json.dumps(data.get('work_experience', [])) if data.get('work_experience') else None
        education_json = json.dumps(data.get('education', [])) if data.get('education') else None
